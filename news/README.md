...
the top of that file), then run `python3 apps/tools/build-news.py` to regenerate
`news.json`. Commit + push and it's live (hub shows the latest 6 across all apps; each app
page shows that app's latest 5).

## Post fields (in News.md)

`date` (YYYY-MM-DD) · `app` (`hexatone`/`pond`/`anima`/`re-deemer`/`voltagecontrol`/`datum`,
or blank = general) ·
